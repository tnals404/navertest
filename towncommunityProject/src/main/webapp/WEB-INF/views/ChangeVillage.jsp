...
<jsp:include page="Header.jsp" />
	...
	<div style="display: flex; margin:20px 0; flex-direction: column; width:100%;">
		<div style="display: flex; justify-content: space-between; align-items: center;">
			<div>
				<c:if test="${town_id == member_town_id}">
				...
				<c:if test="${town_id != member_town_id}">
					<button class="button" style="padding:5px 10px;" onclick="document.location.href='/changeVillageResult?town_id=${town_id}'">
				   		<img style="height: 15px;" color="black"src="img/홈버튼.png">
				   		홈으로
				   	</button>
	...
	$.ajax({
		...
		success:function(response){
			for(let i =0; i<response.length;i++){
				$("#result").append("<button class='button' style='margin: 10px 10px 0 0; padding: 5px 10px; font-size: 20px;' onclick='document.location.href=\"/changeVillageResult?town_id="+response[i].town_id+"\"'>"+response[i].address+"</button>");
			}
		},
		error:function(){}
	});//ajax
